...
- 可視セクション構造・コピー・CSS は v7.7 から変更なし
...
| G-6 | **Nav / Header** | `#nav` | 固定ヘッダー。logo + 6リンク(**v7.5**:About / For Whom / Services / Method / FAQ / Contact)+ ご相談へ CTA |
...
### §1. About the Founder(Chapter 01)

`class="about-section" id="#about"`

| アドレス | 要素 | 内容 |
|---|---|---|
| §1-head | `.about-head-centered`(中央揃え、grid 外) | Chapter 01 ラベル + タイトル「はじめまして、njoooy です。」 |
| §1-img | `.about-image` | 左カラム(sticky)、Portrait プレースホルダー(SVG、差し替え予定) |
| §1-body-1 | `.about-body`(上段) | 京都で[TBD年数]→ 2026年4月東京。**21歳**の自己紹介 |
| §1-pull | `.about-pull` | Pull quote「飲む日も、飲まない日も、同じ温度で一杯を渡せる場所を。」 |
| §1-body-2 | `.about-body`(下段) | 出張バー・メニュー開発・店舗アドバイザリーの提供範囲 |
| §1-TL | `.about-timeline` | 歩み:2024 / 2025 / 2026.04(current、gold breathing dot)/ 2026+ |

---

### §2. For Whom(Chapter 02 — こんな場面に)— **v7.6 で カードグリッド化**

`class="for-whom" id="#for-whom"`、6ケースは **2カラムカードグリッド**(`.case-grid` → `.case-card`)

| アドレス | 要素 | 内容 | SVG 状態 |
|---|---|---|---|
| §2-head | `.fw-head`(中央揃え) | Chapter 02 ラベル + タイトル「njoooy が迎えにゆく、6つの場面。」+ 説明 | — |
...
### §3. Services / Archive 01–04(Chapter 03)

`class="archive" id="#services"`

| アドレス | 要素 | 内容 |
|---|---|---|
| §3-head | `.archive-head`(中央揃え) | Chapter 03 ラベル + タイトル「njoooy が いま開いている、ひらかれたカウンター。」+ 説明 |
| §3-WHY | `.why-njoooy` | **Why njoooy — 選ばれる3つの理由**:01 ノンアルも同じ設計精度で / 02 京都ホテルバー基礎 × 東京柔軟性 / 03 少人数特化 × バイリンガル対応 |
| §3-01 | **Folio 01 njoooy Bar**(出張バー) | Packages: Private ¥50,000〜 / Corporate ¥130,000〜 / Premium ¥280,000〜 |
| §3-01-SM | Folio 01 内 **Sample Menu**(3種) | 01 Kyoto Old Fashioned / 02 Hinoki Garden / 03 Tokyo Midnight Martini — horizontal scroll |
| §3-02 | **Folio 02 njoooy Menu**(メニュー開発) | Packages: Starter ¥130,000 / Standard ¥480,000 / Non-Alc Package ¥380,000 / Retainer ¥130,000/月 |
| §3-03 | **Folio 03 njoooy Counsel**(店舗相談) | Packages: 単発(2h)¥35,000 / 月次伴走 ¥100,000/月 |
| §3-04 | **Folio 04 njoooy Class**(研修プログラム)— プレースホルダー | Coming Soon 2026 夏。告知希望 CTA |

---

### §4. ご依頼の流れ / Business Flow(Chapter 04)

`class="process" id="#process"`

| アドレス | 要素 | 内容 |
|---|---|---|
| §4-head | `.process-head`(中央揃え) | Chapter 04 ラベル + タイトル「ご相談から、当日・そしてその先まで。」+ 説明 |
| §4-01 | Step 01 | Inquiry(お問い合わせ)— DM/メール、24h 以内返信 |
| §4-02 | Step 02 | Hearing(ヒアリング)— 15-30分 Online、48h 以内実施 |
| §4-03 | Step 03 | Proposal(ご提案・お見積り)— 2-3案 + 見積 PDF、48h 以内送付 |
| §4-04 | Step 04 | Contract(契約)— 業務委託契約書 + 前金50% |
| §4-05 | Step 05 | Event Day(当日)— 1-2h 前現地入り、設営→サービス→撤収 |
| §4-06 | Step 06 | Follow-up(フォローアップ)— 残額請求書を7日以内発行 |
| §4-M | **Marquee**(Process 直後) | 業務の流れから、一杯の設計へ / 味を聞く、構造を組む、手から届ける / The njoooy Method |

---

### §5. 一杯の哲学 / The njoooy Method(Chapter 05)

`class="method" id="#method"`

| アドレス | 要素 | 内容 |
|---|---|---|
| §5-head | `.method-head`(中央揃え) | Chapter 05 ラベル + kicker「The njoooy Method」+ タイトル「一杯を、こうして組みます。」+ 説明(Chapter 04 との差別化) |
| §5-01 | Step One | **味を聞く** / Tasting the scene. — tags: Scene / Guests / Mood / Dietary |
| §5-02 | Step Two | **構造を組む** / Building the structure. — tags: Balance / Aroma / Arc / Pacing |
| §5-03 | Step Three | **手から届ける** / Serving, hand to hand. — tags: Counter / Dialogue / Rhythm / Care |
| §5-sig | `.method-sign-brand` | 署名風「— njoooy  Method, Vol. 001」+ footnote |
| §5-M | **Marquee dark**(Method 直後) | 味を聞く／構造を組む／手から届ける |

---

### 🚫 §? 京都での日々 / Kyoto Era(Chapter 06、**v7.5 で HTML コメントアウト・非表示**)

`<!-- <section class="past" id="past"> ... </section> -->`

**現状:HTML 内に保存されているが、ブラウザでは非表示。** 1件目の東京案件が獲得できた時点で再有効化予定。
...
コメント内の Kyoto ラベルは **「Chapter 06」に pre-set** 済み(再有効化時の修正不要)。
...
| §6-toggle | `.faq-toggle`(**v7.5 新**) | SVG 下向き矢印、ホバーで wine 塗り + halo + Q 変色、open 時 180deg 回転 |
...
| §7-01 | Instagram カード | @njoooy(Primary — DM) |
| §7-02 | Email カード | hello@njoooy.com |
| §7-form | **Contact フォーム**(`.contact-form`、**v7.5 新**) | 4 field(お名前 / Email / ご相談種別 select / ご相談内容 textarea)+ 送信ボタン。`action="[TBD: Formspree endpoint]"`、endpoint 差替で稼働。onsubmit 仮 alert fallback |
...
### 2.5.3 公開前に追加予定(v7.8 では未作成)

| 予定パス | 内容 | 優先度 |
|---|---|---|
| `/favicon.svg` | SVG 版 favicon(最優先で表示) | 公開後 Week 1 |
| `/favicon-32x32.png` | PNG フォールバック | 公開後 Week 1 |
| `/apple-touch-icon.png` | 180×180 iOS 用 | 公開後 Week 1 |
| `/og-image.png` | 1200×630 SNS シェア画像 | 公開後 Week 1 |
| `/legal/terms.html` | 利用規約 | 必要性判断後 |
...
- §0-5 Fact 値の最終調整
- §1 About:京都の年数、2024/2025 タイムラインのエピソード
- §3-WHY 各項目の最終推敲
- §3-01〜03 の price(インボイス登録後、消費税の明記)
- §3-04 Class の開講日
- §4-01〜06 各 Step の desc 最終確認
- §5-01〜03 の step 名・命名の最終確認
- §6-Q01〜Q06 の答えの最終確認
- §7-01 Instagram アカウント実在化
- §7-form の Formspree endpoint 差替
...
*本マップは v7.5 時点。セクション追加・削除があればバージョン更新時にこのファイルも追随する。*
